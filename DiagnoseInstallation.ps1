...
foreach ($bundle in $found) {
    ...
    try {
        ...
        if ($version -eq "1.5.3") { Add-Pass "Manifest version is 1.5.3." } else { Add-Warn "Manifest is not version 1.5.3." }
...
if ($loaderKeys.Count -eq 0) {
    ...
    foreach ($loaderKey in $loaderKeys) {
        ...
        if ([string]::IsNullOrWhiteSpace($loader)) {
            ...
            Add-Pass "Loader points to the trusted v1.5.3 bundle: $loader"
        } else {
