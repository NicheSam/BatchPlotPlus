...
- 依使用者要求移除疑似跨圖紙阻擋、物件外包範圍驗證及其他圖框排除；框選到的完整物件直接輸出。
- 每個矩形／封閉聚合線仍各輸出一檔，保留 UCS／視圖還原，不進行裁切。
- 改為只讀取本次處理邊界，不因未選取的其他邊界造成提前失敗。PDF 與共用邏輯不變。

## 1.5.2 — 本機驗證版

- 依後續授權，DWG 分頁開放矩形與封閉聚合線，使用實際輪廓；PDF 與共用邏輯保持 1.4.4 基準。
- 跨圖紙外包範圍檢查會停止受影響分頁，命令列與記錄指出物件 Handle；保留 WB 完整物件語意，不自動裁切或炸開。
- 還原拆圖前 UCS／視圖，取消選取後停止後續分頁。
- AutoCAD 2023 通過合成案例的分檔、凹形與跨框阻擋；原圖副本確認過大圖塊遭攔下，不宣稱已自動修復來源圖。
- 本機已安裝並核對檔案與登錄，重開後正式載入待確認；尚未發布 Release，其他年版尚未實機驗證。
