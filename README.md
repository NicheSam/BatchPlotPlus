...
BatchPlotPlus 是供 AutoCAD 2021–2025（Windows 64 位元）使用的繁體中文批次出圖工具。本分支為 **1.5.0 升級候選版**，沿用原專案、產品識別與指令；各版本桌面實機驗證仍待完成。
...
- **完整新 DLL 的桌面 GUI、PDF、拆圖、首次開圖和各版本實機尚未通過驗證；請勿把建置成功視為完成驗收。**

詳細範圍、遷移方式與驗證缺口見 [1.5 升級紀錄](docs/upgrade-1.5.md)。本分支套件尚未發布到 GitHub Releases；下方 1.4.4 截圖及歷史驗證保留作為既有版本紀錄。
